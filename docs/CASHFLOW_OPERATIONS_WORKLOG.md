...
- 2026-04-02: Expand the legacy FX settlement backfill to also repair old `manual sell` rows that were historically created instead of `card_payment`. Matching stays conservative: only one unambiguous same-day expense candidate is linked, and then the trade is reclassified to `card_payment`.
- 2026-04-02: Fix kebab actions for `Операции -> Денежный поток` and `Валюта` when menus are portaled to `document.body`. Action handlers now listen on document-level too, so menu buttons work after the popover leaves the table DOM subtree.
- 2026-04-02: Clarify dashboard semantics: `Профицит/Дефицит` stays period cashflow in the analytics KPI block, while the `Валютный портфель` panel no longer mixes period cashflow with FX result. The top portfolio card now shows current portfolio valuation in base currency, with separate invested/base and total FX result context; tracked currency cards show quantity, current rate, and approximate base-currency valuation.
- 2026-04-02: Separate `Операционный результат` from `Денежный поток` across dashboard/analytics/operations. Dashboard and analytics KPI rows now show both metrics explicitly; trend legend/tooltip and calendar headers now call unified flow `Денежный поток` instead of `Профицит/Дефицит`; operations summary is mode-aware (`Операционный результат` in operations mode, `Денежный поток` in money-flow mode).
- 2026-04-02: Simplify the dashboard `Валютный портфель` block. Remove the heavy KPI grid from dashboard-level presentation, keep the portfolio valuation card plus per-currency quantity/rate/base-valuation cards, and move invested/unrealized/realized/total FX-result details into compact secondary chips.
- 2026-04-02: Refine analytics semantics and density. In `Структура`, mode `Все` now presents income and expense as explicitly mixed category structure instead of an unlabeled common total, with split income/expense context in the donut center and type badges in the list. In `Календарь`, day cells no longer inline-expand with `Долг` / `FX` rows; cells stay compact and move debt/fx detail into tooltip/meta while preserving unified `Денежный поток` totals.
- 2026-04-02: Promote analytics/dashboard structure mode `Все` from a single mixed donut to a split visual layout. Income and expense now render as two side-by-side mini-donuts with independent shares, while the shared list keeps type badges for cross-checking. `Долги` and `FX` stay outside category structure on purpose; they remain part of cashflow analytics, not category taxonomy.
- 2026-04-02: Unify `Операции -> Денежный поток` row behavior with the main operations list. Row-click now opens the primary edit flow for every source type (`operation`, `debt`, `fx`), kebab menus are consistent and actionable for each source, and FX rows show the traded asset amount under the base-currency sum (`Куплено/Продано X USD`).
- 2026-04-02: Reduce noisy dashboard partial-refresh failures on section switch. `switchSection("dashboard")` no longer double-loads the plans block through both `loadDashboard()` and `loadDashboardOperations()`, and optional dashboard jobs such as analytics preview stop raising the generic `Не удалось полностью обновить дашборд` toast when the main dashboard data loaded successfully.
