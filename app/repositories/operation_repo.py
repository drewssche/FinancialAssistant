...
class OperationRepository:
    ...
    def restore(self, *, user_id: int, snapshot: dict) -> Operation:
        operation = Operation(
            ...
            fx_rate_source=snapshot.get("fx_rate_source"),
            fx_bank_code=snapshot.get("fx_bank_code"),
            fx_bank_name=snapshot.get("fx_bank_name"),
            fx_bank_channel=snapshot.get("fx_bank_channel"),
            fx_rate_kind=snapshot.get("fx_rate_kind"),
            fx_rate_scale=int(snapshot.get("fx_rate_scale") or 1),
            fx_rate_date=(date.fromisoformat(snapshot["fx_rate_date"]) if snapshot.get("fx_rate_date") else None),
            fx_quoted_at=(datetime.fromisoformat(snapshot["fx_quoted_at"]) if snapshot.get("fx_quoted_at") else None),
            fx_fetched_at=(datetime.fromisoformat(snapshot["fx_fetched_at"]) if snapshot.get("fx_fetched_at") else None),
            fx_rate_stale=bool(snapshot.get("fx_rate_stale", False)),
            fx_payment_mode=snapshot.get("fx_payment_mode") or "valuation",
