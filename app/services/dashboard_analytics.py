...
class DashboardAnalyticsService:
    ...
    def build_category_amount_allocations(
        self,
        *,
        operations: list,
    ...
    def load_category_maps_for_allocations(self, allocations: list[dict]) -> tuple[dict[int, str], dict[int, str], dict[int, int | None], dict[int, str | None]]:
        category_ids = {
    ...
    def resolve_period_bounds(self, **kwargs) -> tuple[date, date]:
        return self.timeline.resolve_period_bounds(**kwargs)

    def month_bounds(self, anchor: date) -> tuple[date, date]:
        return self.timeline.month_bounds(anchor)

    def percent_change(self, current: Decimal, previous: Decimal) -> float | None:
    ...
    def get_highlights(
        ...
        *,
        ...
        if period == "month" and month_anchor is not None:
            resolved_from, resolved_to = self.month_bounds(month_anchor)
        else:
        ...
        month_start, month_end = self.month_bounds(resolved_from)
        ...
        current_category_allocations = self.build_category_amount_allocations(
            operations=operations,
            receipt_items_by_operation=current_receipt_items,
        )
        previous_category_allocations = self.build_category_amount_allocations(
        ...
        for key, current_item in current_position_stats.items():
            ...
            change_pct = self.percent_change(curr_avg, prev_avg)
        ...
        return {
            ...
            "income_change_pct": self.percent_change(income_total, prev_income_total),
            "expense_change_pct": self.percent_change(expense_total, prev_expense_total),
            "balance_change_pct": self.percent_change(balance, prev_balance),
            "operations_change_pct": self.percent_change(Decimal(len(operations)), Decimal(len(prev_operations))),
            "category_breakdown": [
                {
                    ...
                    "change_pct": self.percent_change(bucket["total_amount"], previous_category_totals[key]),
