...
def test_dashboard_analytics_calendar_includes_historical_currency_value_in_balance(client: TestClient):
    client.post(
        "/api/v1/operations",
        json={
            "kind": "income",
    ...
    assert rate_first.status_code == 200
    trade = client.post(
        "/api/v1/currency/trades",
        json={
            "side": "buy",
    ...
    assert rate_second.status_code == 200

    response = client.get("/api/v1/dashboard/analytics/calendar", params={"month": "2026-03"})
    assert response.status_code == 200
    payload = response.json()

    assert payload["flow_balance"] == "80.00"
    assert payload["operation_balance"] == "130.00"
    assert payload["currency_value"] == "34.00"
    assert payload["balance"] == "164.00"

    target_day = next(day for week in payload["weeks"] for day in week["days"] if day["date"] == "2026-03-03")
    assert target_day["flow_balance"] == "-20.00"
    assert target_day["operation_balance"] == "130.00"
    assert target_day["currency_value"] == "32.00"
    assert target_day["balance"] == "162.00"

    target_week = next(week for week in payload["weeks"] if any(day["date"] == "2026-03-03" for day in week["days"]))
    assert target_week["currency_value"] == "34.00"
    assert target_week["balance"] == "164.00"


def test_dashboard_analytics_calendar_year_balance_uses_month_end_historical_currency_value(client: TestClient):
    client.post(
        "/api/v1/operations",
        json={
            "kind": "income",
    ...
    assert mar["flow_balance"] == "-40.00"
    assert mar["operation_balance"] == "60.00"
    assert mar["currency_value"] == "34.00"
    assert mar["balance"] == "94.00"


def test_dashboard_analytics_calendar_includes_historical_debt_exposure_in_balance(client: TestClient):
    client.post(
        "/api/v1/operations",
        json={
            "kind": "income",
    ...
    repaid = client.post(
        f"/api/v1/debts/{debt.json()['id']}/repayments",
        json={
            "amount": "20.00",
    ...
    assert response.status_code == 200
    payload = response.json()
    assert payload["debt_value"] == "30.00"
    assert payload["balance"] == "130.00"

    day_before_repayment = next(day for week in payload["weeks"] for day in week["days"] if day["date"] == "2026-03-03")
    assert day_before_repayment["debt_value"] == "50.00"
    assert day_before_repayment["balance"] == "150.00"

    day_after_repayment = next(day for week in payload["weeks"] for day in week["days"] if day["date"] == "2026-03-04")
    assert day_after_repayment["debt_value"] == "30.00"
    assert day_after_repayment["balance"] == "130.00"


def test_dashboard_analytics_calendar_debt_mutation_invalidates_cache(client: TestClient):
    debt = client.post(
        "/api/v1/debts",
        json={
            "counterparty": "Петр",
    ...
    initial = client.get("/api/v1/dashboard/analytics/calendar", params={"month": "2026-03"})
    assert initial.status_code == 200
    assert initial.json()["debt_value"] == "-40.00"

    warm = client.get("/api/v1/dashboard/analytics/calendar", params={"month": "2026-03"})
    assert warm.status_code == 200
    assert warm.json()["debt_value"] == "-40.00"
    ...
    after_repayment = client.get("/api/v1/dashboard/analytics/calendar", params={"month": "2026-03"})
    assert after_repayment.status_code == 200
    assert after_repayment.json()["debt_value"] == "-25.00"
