...
def test_dashboard_navigation_ignores_stale_section_loads():
    ...
    assert "if (dashboardOptionalLoadPromise) {" in dashboard
    assert "const optionalRequestSignal = dashboardOptionalLoadController.signal" in dashboard
    assert "loadPlans?.({ signal: optionalRequestSignal })" in dashboard
    assert "loadDebtPreview({ limit: 6, signal: optionalRequestSignal })" in dashboard
    load_dashboard_body = dashboard.split("async function loadDashboard() {", 1)[1].split("async function loadDashboardPlans()", 1)[0]
    assert "dashboardOptionalLoadController.abort();" not in load_dashboard_body
...
def test_hot_paths_use_local_action_getters_instead_of_direct_global_calls():
    ...
    period_control_utils = (REPO_ROOT / "static" / "js" / "app-period-control-utils.js").read_text(encoding="utf-8")
    dashboard_data = (REPO_ROOT / "static" / "js" / "app-dashboard-data.js").read_text(encoding="utf-8")
    plans_recurrence = (REPO_ROOT / "static" / "js" / "app-features-plans-recurrence.js").read_text(encoding="utf-8")
    plans_render = (REPO_ROOT / "static" / "js" / "app-features-plans-render.js").read_text(encoding="utf-8")
    plans_dashboard = (REPO_ROOT / "static" / "js" / "app-features-plans-dashboard.js").read_text(encoding="utf-8")
    ...
    analytics_currency = (REPO_ROOT / "static" / "js" / "app-features-analytics-currency.js").read_text(encoding="utf-8")
    analytics_currency_trades = (
        REPO_ROOT / "static" / "js" / "app-features-analytics-currency-trades.js"
    ).read_text(encoding="utf-8")
    analytics_currency_chart = (
        REPO_ROOT / "static" / "js" / "app-features-analytics-currency-chart.js"
    ).read_text(encoding="utf-8")
    currency = (REPO_ROOT / "static" / "js" / "app-features-currency.js").read_text(encoding="utf-8")
    currency_performance = (
        REPO_ROOT / "static" / "js" / "app-features-currency-performance.js"
    ).read_text(encoding="utf-8")
    currency_trades = (REPO_ROOT / "static" / "js" / "app-features-currency-trades.js").read_text(encoding="utf-8")
    analytics_highlights = (REPO_ROOT / "static" / "js" / "app-features-analytics-highlights.js").read_text(encoding="utf-8")
    ...
    op_modal_debt_form = (REPO_ROOT / "static" / "js" / "app-features-operation-modal-debt.js").read_text(encoding="utf-8")
    op_modal_fx_settlement = (REPO_ROOT / "static" / "js" / "app-features-operation-modal-fx-settlement.js").read_text(encoding="utf-8")
    op_modal_currency = (REPO_ROOT / "static" / "js" / "app-features-operation-modal-currency.js").read_text(encoding="utf-8")
    ...
    init_operations = (REPO_ROOT / "static" / "js" / "app-init-features-operations.js").read_text(encoding="utf-8")
    ...
    assert "function getNavigationActions()" in session_feature
    assert "function getOperationsFeature()" in session_feature
    ...
    assert "function getBulkUi()" in init_core
    assert "getBulkUi().openBatchCreateModal?.()" in init_core
    assert "getBulkUi().openBatchCategoryModal?.()" in init_core
    assert "getBulkUi().openBatchItemTemplateModal?.()" in init_core
    assert "getCategoryActions().openCreateGroupModal?.()" in init_core
    assert "function openCreateGroupModal()" in categories_ui
    assert "openCreateGroupModal: categoryUi.openCreateGroupModal" in categories_js
    assert "function getCategoryActions()" in init_startup
    assert "function getNavigationActions()" in init_startup
    ...
    assert "function getOperationsFeature()" in init_operations
    ...
    assert "function getOperationsPeriodControls(" in init_features
    ...
    assert 'registerRuntimeModule?.("period-control-utils"' in period_control_utils
    ...
    assert 'registerRuntimeModule?.(' in analytics_currency_trades
    assert '"analytics-currency-trades-factory"' in analytics_currency_trades
    assert 'getRuntimeModule?.("analytics-currency-trades-factory")' in analytics_currency
    assert "function loadAnalyticsCurrencyTradesPage(" not in analytics_currency
    ...
    assert '"currency-performance-factory"' in currency_performance
    assert '"currency-trades-factory"' in currency_trades
    assert 'getRuntimeModule?.("currency-performance-factory")' in currency
    assert 'getRuntimeModule?.("currency-trades-factory")' in currency
    assert "function renderPerformanceChart(" not in currency
    assert "function loadCurrencyTradesPage(" not in currency
    ...
    assert 'registerRuntimeModule?.("analytics-period-controls"' in analytics_period_controls
    ...
    assert 'getRuntimeModule?.("analytics-period-controls")' in analytics_init
    ...
    assert "function getPickerActions()" in pickers_init
    assert "...getOperationModal()" in pickers_init
    assert "...getItemCatalogFeature()" in pickers_init
    ...
    assert 'registerRuntimeModule?.("plans-recurrence"' in plans_recurrence
    assert 'registerRuntimeModule?.("plans-render"' in plans_render
    assert 'registerRuntimeModule?.("plans-dashboard"' in plans_dashboard
    assert 'registerRuntimeModule?.("operations-period-controls"' in operations_period_controls
    assert 'getRuntimeModule?.("period-control-utils")' in operations_period_controls
    assert 'getRuntimeModule?.("period-control-utils")' in analytics_period_controls
    assert 'getRuntimeModule?.("period-control-utils")' in plans_dashboard
    assert "function shiftPeriodBounds(" in period_control_utils
    assert "function parseIsoDate(" not in operations_period_controls
    assert "function parseIsoDate(" not in analytics_period_controls
    assert 'getRuntimeModule?.("operations-period-controls")' in init_features
    assert 'getRuntimeModule?.("plans-recurrence")' in plans
    assert 'getRuntimeModule?.("plans-render")' in plans
    assert 'getRuntimeModule?.("plans-dashboard")' in plans
    ...
    assert 'registerRuntimeModule?.("navigation", api)' in section_ui
    assert 'getRuntimeModule?.("navigation")' in session_auth
    assert 'getRuntimeModule?.("navigation")' in analytics
    assert 'getRuntimeModule?.("navigation")' in operations
    assert "showSection" not in operations
    ...
    assert 'getRuntimeModule?.("operation-modal-debt-factory")' in op_modal
    assert 'getRuntimeModule?.("operation-modal-fx-settlement-factory")' in op_modal_currency
    assert 'getRuntimeModule?.("operation-modal-currency-factory")' in op_modal
    ...
    assert "window.App.operationModal =" not in op_modal
    ...
    assert "window.App.createOperationModalDebtFeature" not in op_modal_debt_form
    assert "window.App.createOperationModalFxSettlementFeature" not in op_modal_fx_settlement
    assert "window.App.createOperationModalCurrencyFeature" not in op_modal_currency
    ...
    assert "window.App.createPlansRecurrenceFeature" not in plans_recurrence
    assert "window.App.createPlansRenderFeature" not in plans_render
    ...
    assert "return window.App.actions || {}" not in pickers_init
    ...
    assert 'if (period !== "all_time")' in analytics_highlights
    ...
    assert "actions.openOperationReceiptModal" not in init_operations
    assert "operationsFeature.openOperationReceiptModal(item);" in init_operations
